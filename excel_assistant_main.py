...
class ExcelAnalyzer:
    ...
    def load_excel(self, file_path):
        ...
        try:
            ...
            if file_path.endswith('.xlsx') or file_path.endswith('.xls'):
                ...
                return "❌ Unsupported file format. Please use .xlsx, .xls, or .csv"

            self.file_name = file_path.split('/')[-1]

            # Initial analysis
            response = f"✅ Successfully loaded '{self.file_name}'!\n\n"
            ...
            response += "💬 What would you like to know about this data?"
    ...
    def detect_missing_values(self):
        ...
        if not has_missing:
            response += "✅ No missing values found! Your data is complete.\n"
        else:
            response += "\n💡 **Suggestion:** Consider filling or removing missing values for better analysis.\n"
    ...
    def show_data_preview(self, rows=5):
        ...
        response = f"👀 **Data Preview (First {rows} rows):**\n\n"
    ...
    def get_column_info(self, column_name):
        ...
        if column_name not in self.df.columns:
            return f"❌ Column '{column_name}' not found. Available columns: {', '.join(self.df.columns.tolist())}"

        col_data = self.df[column_name]

        response = f"📋 **Analysis of '{column_name}':**\n\n"
...
print("✅ ExcelAnalyzer class created successfully!")
...
class ExcelChatBot:
    ...
    def get_help_message(self):
        """Return help message"""
        help_text = """
🤖 **Excel Analysis Assistant - Help Guide**
...
# Initialize chatbot
bot = ExcelChatBot()
print("✅ Excel ChatBot initialized!")
print("\n" + "="*60)
print("🤖 Excel Analysis Assistant Ready!")
print("="*60)
...
print("✅ Sample employee dataset created!")
print(f"📊 Dataset contains {len(df_employees)} employees with {len(df_employees.columns)} columns")
print(f"\n📋 Columns: {', '.join(df_employees.columns.tolist())}")
print(f"\n👀 Preview:")
...
class ExcelAnalyzer:
    ...
    def detect_missing_values(self):
        ...
        if not has_missing:
            response += "✅ Great news! No missing values found. Your data is complete.\n"
        else:
            response += "\n💡 **Recommendations:**\n"
    ...
    def calculate_correlations(self):
        ...
        response = "🔗 **Correlation Analysis**\n\n"
        response += "📚 Understanding Correlations:\n"
        response += "  • 1.0 = Perfect positive correlation\n"
        response += "  • -1.0 = Perfect negative correlation\n"
        response += "  • 0.0 = No correlation\n\n"
        ...
        if strong_positive:
            response += "⚡ **Strong Positive Correlations Found:**\n"
        ...
        if not strong_positive and not strong_negative:
            response += "ℹ️ No particularly strong correlations detected (threshold: ±0.7)\n"
    ...
    def get_column_info(self, column_name):
        ...
        response = f"📋 **Detailed Analysis of '{column_name}':**\n\n"
...
class ExcelChatBot:
    ...
    def process_message(self, user_message):
        ...
        if any(word in user_message_lower for word in ['summary', 'statistics', 'describe', 'stats']):
            ...
            response = "🤔 I'm not quite sure what you're asking for. Here are some things you can try:\n\n"
            response += "  • 'Show summary statistics'\n"
            response += "  • 'Check for missing values'\n"
            response += "  • 'Calculate correlations'\n"
            response += "  • 'Preview the data'\n"
            response += "  • 'Analyze column Salary'\n"
            response += "  • 'Show department analysis'\n"
...
print("="*70)
print("🤖 EXCEL ANALYSIS ASSISTANT")
print("="*70)
print()
...
print("✅ Chat function ready! Use: chat('your message')")
print("\nTry these commands:")
...
class AdvancedExcelAnalyzer:
    ...
    def filter_data(self, column, operator, value):
        ...
        try:
            ...
            if not matching_cols:
                return f"❌ Column '{column}' not found. Available: {', '.join(self.df.columns.tolist())}"
            ...
            response += f"🔍 Filter: {col_name} {operator} {value}\n"
            response += f"📊 Results: {len(self.df_filtered)} rows (from {len(self.df)} total)\n\n"

            if len(self.df_filtered) > 0:
                response += "**Preview of filtered data:**\n"
                response += self.df_filtered.head(5).to_string(index=False)
            else:
                response += "⚠️ No rows match this filter."
    ...
    def reset_filters(self):
        ...
        return f"✅ All filters cleared! Back to {len(self.df)} rows."

    def show_active_filters(self):
        """Show currently active filters"""
        if not self.active_filters:
            return "ℹ️ No active filters. Showing all data."

        response = "🔍 **Active Filters:**\n"
        for i, filt in enumerate(self.active_filters, 1):
            response += f"  {i}. {filt}\n"
        response += f"\n📊 Showing {len(self.df_filtered)} of {len(self.df)} rows"
    ...
    def group_by_column(self, group_col, agg_col=None, agg_func='mean'):
        ...
        try:
            ...
            response = f"📊 **Grouped Analysis: {group_col_name}**\n\n"
    ...
    def create_bar_chart(self, column, title=None):
        ...
        try:
            ...
            return f"✅ Bar chart created for '{col_name}'"
    ...
    def create_histogram(self, column, bins=20):
        ...
        try:
            ...
            return f"✅ Histogram created for '{col_name}'"
    ...
    def create_pie_chart(self, column):
        ...
        try:
            ...
            return f"✅ Pie chart created for '{col_name}'"
    ...
    def find_outliers(self, column):
        ...
        try:
            ...
            response = f"🔍 **Outlier Analysis: {col_name}**\n\n"
            response += f"📊 Statistics:\n"
            response += f"  • Q1 (25th percentile): {Q1:.2f}\n"
            response += f"  • Q3 (75th percentile): {Q3:.2f}\n"
            response += f"  • IQR: {IQR:.2f}\n"
            response += f"  • Lower Bound: {lower_bound:.2f}\n"
            response += f"  • Upper Bound: {upper_bound:.2f}\n\n"
            response += f"⚠️ **Outliers Found: {len(outliers)}**\n"
    ...
    def compare_groups(self, group_col, numeric_col):
        ...
        try:
            ...
            response = f"📊 **Comparison: {num_name} across {group_name}**\n\n"
    ...
    def detect_missing_values(self):
        ...
        if not has_missing:
            response += "✅ Great news! No missing values found.\n"
        else:
            response += "\n💡 **Recommendations:**\n"
    ...
    def calculate_correlations(self):
        ...
        if strong_positive:
            response += "⚡ **Strong Positive Correlations:**\n"
            for corr in strong_positive:
                response += f"  • {corr}\n"
    ...
    def show_data_preview(self, rows=10):
        ...
        response += f"\n📊 Showing {min(rows, len(self.df_filtered))} of {len(self.df_filtered)} rows"

        return response


print("✅ Advanced Excel Analyzer created with filtering, grouping, and charts!")
...
print("✅ Natural Language Understanding Engine created!")
...
class AdvancedExcelChatBot:
    ...
    def process_message(self, user_message):
        ...
        if self.analyzer.df is None and 'load' not in user_message.lower():
            response = "❌ Please load an Excel file first using: bot.analyzer.load_excel('filename.xlsx')"
    ...
    def _handle_filter(self, parsed):
        ...
        if not columns:
            return "❌ Please specify a column to filter. Example: 'Filter Department equals Sales'"

        if value is None:
            return "❌ Please specify a value to filter by. Example: 'Show employees where Salary > 50000'"
    ...
    def _handle_group(self, parsed):
        ...
        if not columns:
            return "❌ Please specify column(s) to group by. Example: 'Group by Department'"
        ...
        return "❌ Unable to process grouping request."
    ...
    def _handle_chart(self, parsed):
        ...
        if not columns:
            return "❌ Please specify column(s) for the chart. Example: 'Create bar chart for Department'"
    ...
    def _handle_outliers(self, parsed):
        ...
        if not columns:
            return "❌ Please specify a numeric column. Example: 'Find outliers in Salary'"
    ...
    def _handle_compare(self, parsed):
        ...
        if len(columns) < 2:
            return "❌ Please specify two columns to compare. Example: 'Compare Salary across Department'"
        ...
        if 'how many' in message_lower or 'count' in message_lower:
            for col in self.analyzer.df.columns:
                if col.lower() in message_lower:
                    ...
                    return f"📊 Count of {col}: {count}"
            return f"📊 Total rows in dataset: {len(self.analyzer.df_filtered)}"

        # Show columns
        if 'columns' in message_lower or 'fields' in message_lower:
            return f"📋 Available columns:\n  • " + "\n  • ".join(self.analyzer.df.columns.tolist())
    ...
    def _get_suggestions(self, message):
        ...
        response = "🤔 I'm not quite sure what you're asking. Here are some suggestions:\n\n"
        response += "**Analysis:**\n"
        response += "  • 'Show summary statistics'\n"
        response += "  • 'Check for missing values'\n"
        response += "  • 'Calculate correlations'\n\n"
        ...
        response += "**Filtering:**\n"
        response += "  • 'Filter Department equals Sales'\n"
        response += "  • 'Show employees where Salary > 50000'\n"
        response += "  • 'Find people in IT department'\n\n"

        response += "**Grouping:**\n"
        response += "  • 'Group by Department'\n"
        response += "  • 'Average Salary by Department'\n"
        response += "  • 'Count employees by City'\n\n"

        response += "**Charts:**\n"
        response += "  • 'Create bar chart for Department'\n"
        response += "  • 'Show histogram of Age'\n"
        response += "  • 'Scatter plot Salary vs Experience'\n\n"

        response += "**Advanced:**\n"
        response += "  • 'Find outliers in Salary'\n"
        response += "  • 'Compare Salary across Department'\n"
    ...
    def get_help_message(self):
        ...
        help_text = """
🤖 **Excel Analysis Assistant - Complete Guide**
...
🔍 **FILTERING DATA:**
  ...
  • "Find people with Age less than 30"
  • "Show only IT department"
  • "Reset filters" (to clear all filters)
...
🔬 **ADVANCED ANALYSIS:**
  ...
  • "Compare Salary across Department"
  • "Analyze column Performance_Rating"

💡 **SMART QUERIES (Natural Language):**
  • "What's the average salary?"
  • "How many employees in Sales?"
  • "Show me IT employees earning over 70000"
  • "Which department has highest average salary?"

🎯 **TIPS:**
  • Be conversational! I understand context
  • Combine filters: "Show Sales employees with Salary > 50000"
  • Chain operations: First filter, then group, then visualize
  • Type 'active filters' to see current filters
  • Type 'reset' to start fresh

🔒 **Privacy:** All processing happens locally. No data leaves your computer!
"""
        return help_text
...
print("✅ Advanced Excel ChatBot with NLP initialized!")
...
print("="*70)
print("🚀 ADVANCED EXCEL ANALYSIS ASSISTANT")
print("="*70)
print()
...
print("✅ Fixed NLP Engine created!")
print("✅ Better intent detection with priority handling!")
...
print("✅ Bot updated with fixed NLP!")

# Reload the data
print("="*70)
print("🚀 ADVANCED EXCEL ANALYSIS ASSISTANT (FIXED)")
print("="*70)
print()
...
print("✅ Excel Formula Generator created!")
...
class SuperAdvancedExcelAnalyzer(AdvancedExcelAnalyzer):
    ...
    def get_comprehensive_statistics(self):
        ...
        response = "📊 **COMPREHENSIVE STATISTICAL ANALYSIS**\n\n"
        ...
        for col in numeric_cols:
            ...
            response += f"{'='*60}\n"
            response += f"📈 {col.upper()}\n"
            response += f"{'='*60}\n\n"

            # Central Tendency
            response += "**📍 CENTRAL TENDENCY:**\n"
            ...
            response += f"  • Standard Deviation: {std_val:.2f}\n"
            response += f"    Excel: {self.formula_gen.stdev_formula(col, start_row, end_row)}\n\n"

            response += f"  • Variance: {var_val:.2f}\n"
            ...
            response += f"  • Q2 (50th percentile/Median): {q2:.2f}\n"
            ...
            response += f"  • IQR (Interquartile Range): {iqr:.2f}\n"
            ...
            response += f"  • Skewness: {skewness:.3f}"
            if skewness > 0:
            ...
            response += f"  • Kurtosis: {kurtosis:.3f}"
            if kurtosis > 0:
    ...
    def get_percentile_analysis(self, column_name):
        ...
        response = f"📊 **PERCENTILE ANALYSIS: {col}**\n\n"
        ...
        for p in percentiles:
            ...
            response += f"  • {p}th percentile: {value:.2f}\n"
    ...
    def get_frequency_distribution(self, column_name, bins=10):
        ...
        response = f"📊 **FREQUENCY DISTRIBUTION: {col}**\n\n"
        ...
        if pd.api.types.is_numeric_dtype(self.df_filtered[col]):
            ...
            response += "💡 First create bins in a separate column, then use FREQUENCY formula!\n"
            ...
            response += f"\n📈 **Excel Formula for each category:**\n"
            for cat in freq.index[:3]:  # Show first 3
                response += f"  • {cat}: {self.formula_gen.countif_formula(col, cat, start_row, end_row)}\n"
    ...
    def get_summary_by_group(self, group_col, numeric_col):
        ...
        response = f"📊 **GROUP ANALYSIS: {num_name} by {group_name}**\n\n"
        ...
        response += "📈 **Excel Formulas for each group:**\n\n"

        for group_value in self.df_filtered[group_name].unique()[:3]:  # Show first 3
            response += f"**{group_name} = {group_value}:**\n"
            response += f"  • Average: {self.formula_gen.averageif_formula(group_name, group_value, num_name, start_row, end_row)}\n"
            response += f"  • Sum: {self.formula_gen.sumif_formula(group_name, group_value, num_name, start_row, end_row)}\n"
            response += f"  • Count: {self.formula_gen.countif_formula(group_name, group_value, start_row, end_row)}\n\n"

        response += "💡 **Try these formulas in your Excel sheet!**\n"
    ...
    def get_z_scores(self, column_name):
        ...
        response = f"📊 **Z-SCORE ANALYSIS: {col}**\n\n"

        response += f"📍 **Statistics:**\n"
        response += f"  • Mean: {mean:.2f}\n"
        response += f"  • Std Dev: {std:.2f}\n\n"

        response += "**Z-Score Interpretation:**\n"
        response += "  • Z > 2: Value is significantly above average\n"
        response += "  • -2 < Z < 2: Value is within normal range\n"
        response += "  • Z < -2: Value is significantly below average\n\n"

        # Count outliers
        outliers_high = (z_scores > 2).sum()
        outliers_low = (z_scores < -2).sum()

        response += f"⚠️ **Outliers Detected:**\n"
        response += f"  • Above normal (Z > 2): {outliers_high}\n"
        response += f"  • Below normal (Z < -2): {outliers_low}\n\n"

        response += f"📈 **Excel Formula for Z-Score:**\n"
        ...
        response += f"💡 **Or use STANDARDIZE function:**\n"
    ...
    def get_confidence_interval(self, column_name, confidence=0.95):
        ...
        response = f"📊 **CONFIDENCE INTERVAL: {col}**\n\n"
        response += f"**{confidence*100:.0f}% Confidence Interval:**\n"
        response += f"  • Lower Bound: {ci[0]:.2f}\n"
        response += f"  • Mean: {mean:.2f}\n"
        response += f"  • Upper Bound: {ci[1]:.2f}\n\n"
        ...
        response += "💡 Then add/subtract this from the mean to get the interval!\n"

        return response


print("✅ Super Advanced Excel Analyzer with comprehensive statistics created!")
print("✅ All responses now include Excel formulas!")
...
class UltraAdvancedChatBot(AdvancedExcelChatBot):
    ...
    def process_message(self, user_message):
        ...
        if self.analyzer.df is None and 'load' not in user_message.lower():
            response = "❌ Please load an Excel file first."
            self.conversation_history.append({'role': 'assistant', 'message': response})
            return response
        ...
        try:
            ...
            if 'comprehensive' in user_lower or 'detailed statistics' in user_lower or 'full stats' in user_lower:
                ...
                if columns:
                    ...
                    response = "❌ Please specify a column. Example: 'Show percentiles for Salary'"

            elif 'frequency' in user_lower or 'distribution' in user_lower:
                if columns:
                    response = self.analyzer.get_frequency_distribution(columns[0])
                else:
                    response = "❌ Please specify a column. Example: 'Show frequency distribution of Age'"
                ...
                if columns:
                    ...
                    response = "❌ Please specify a column. Example: 'Calculate z-scores for Salary'"

            elif 'confidence interval' in user_lower:
                if columns:
                    response = self.analyzer.get_confidence_interval(columns[0])
                else:
                    response = "❌ Please specify a column. Example: 'Confidence interval for Salary'"
                ...
                if len(columns) >= 2:
                    ...
                    response = "❌ Please specify columns. Example: 'Summary of Salary by Department'"
    ...
    def get_enhanced_help_message(self):
        ...
        help_text = """
🤖 **ULTRA ADVANCED EXCEL ANALYSIS ASSISTANT**

📊 **COMPREHENSIVE STATISTICS:**
  • "Show comprehensive statistics" - Full statistical analysis with Excel formulas
  • "Detailed statistics" - Complete breakdown by column
  • "Percentiles for [column]" - Show percentile analysis
  • "Frequency distribution of [column]" - Show frequency table
  • "Calculate z-scores for [column]" - Find z-scores and outliers
...
📈 **RELATIONSHIPS & GROUPING:**
  • "Calculate correlations"
  • "Compare [numeric_col] across [group_col]" - Compare stats by group
  • "Group by [column]" - Count by group
  • "Average [numeric_col] by [group_col]" - Aggregate by group
...
🎯 **FILTERING DATA:**
  • "Filter [column] [operator] [value]" (e.g., "Filter Department equals Sales")
  • "Show employees where Salary > 60000"
  • "Reset filters" (to clear all filters)
  • "Show active filters"

💡 **NATURAL LANGUAGE TIPS:**
  • Ask questions like "What's the average salary?", "How many in IT?", etc.
  • Combine actions: "Filter by Department Sales and show summary"
  • Use 'help' for this guide

🔒 **Privacy:** All analysis is performed offline within Colab. Your data is safe!
"""
        return help_text
...
print("✅ Ultra Advanced ChatBot completed!")

# Initialize the Ultra Advanced Bot

bot = UltraAdvancedChatBot()
print("✅ Ultra Advanced Excel ChatBot initialized!")
print("✅ Now includes comprehensive statistics with Excel formulas!")

# Load the employee data

print("="*70)
print("🚀 ULTRA ADVANCED EXCEL ANALYSIS ASSISTANT")
print("="*70)
print()
...
class HypothesisTestingEngine:
    ...
    def t_test_independent(self, df, group_col, value_col, group1, group2):
        ...
        response = "📊 **INDEPENDENT T-TEST**\n\n"
        response += f"**Comparing: {value_col} between {group1} and {group2}**\n\n"

        response += "**Group Statistics:**\n"
        response += f"  {group1}:\n"
        response += f"    • Mean: {mean1:.2f}\n"
        response += f"    • Std Dev: {std1:.2f}\n"
        response += f"    • Sample Size: {n1}\n\n"

        response += f"  {group2}:\n"
        response += f"    • Mean: {mean2:.2f}\n"
        response += f"    • Std Dev: {std2:.2f}\n"
        response += f"    • Sample Size: {n2}\n\n"

        response += "**Test Results:**\n"
        response += f"  • t-statistic: {t_stat:.4f}\n"
        response += f"  • p-value: {p_value:.4f}\n\n"

        # Interpretation
        response += "**Interpretation:**\n"
        if p_value < 0.05:
            response += f"  ✅ SIGNIFICANT DIFFERENCE (p < 0.05)\n"
            response += f"  The means are statistically different between {group1} and {group2}\n\n"
        else:
            response += f"  ❌ NO SIGNIFICANT DIFFERENCE (p >= 0.05)\n"
        ...
        response += "  • 2 = two-tailed test\n"
        response += "  • 2 = independent samples (type 2)\n\n"

        response += "💡 **How to do this in Excel:**\n"
    ...
    def t_test_paired(self, df, col1, col2):
        ...
        response = "📊 **PAIRED T-TEST**\n\n"
        response += f"**Comparing: {col1} vs {col2}**\n\n"

        response += "**Statistics:**\n"
        response += f"  • Mean of {col1}: {mean1:.2f}\n"
        response += f"  • Mean of {col2}: {mean2:.2f}\n"
        response += f"  • Mean Difference: {diff_mean:.2f}\n"
        response += f"  • Sample Size: {len(data1)}\n\n"
        ...
        response += "  • 2 = two-tailed test\n"
        response += "  • 1 = paired samples (type 1)\n\n"

        response += "💡 **Use Case:** Before/After comparisons, matched pairs\n"
    ...
    def chi_square_test(self, df, col1, col2):
        ...
        response = "📊 **CHI-SQUARE TEST OF INDEPENDENCE**\n\n"
        ...
        response += "**Test Results:**\n"
        response += f"  • Chi-square statistic: {chi2:.4f}\n"
        response += f"  • p-value: {p_value:.4f}\n"
        response += f"  • Degrees of freedom: {dof}\n\n"

        response += "**Interpretation:**\n"
        if p_value < 0.05:
            response += f"  ✅ SIGNIFICANT RELATIONSHIP (p < 0.05)\n"
            response += f"  {col1} and {col2} are NOT independent\n"
            response += f"  There IS a relationship between these variables\n\n"
        else:
            response += f"  ❌ NO SIGNIFICANT RELATIONSHIP (p >= 0.05)\n"
    ...
    def anova_test(self, df, group_col, value_col):
        ...
        response = "📊 **ONE-WAY ANOVA**\n\n"
        response += f"**Testing: {value_col} across {group_col}**\n\n"

        response += "**Group Statistics:**\n"
        for group, data in zip(groups, group_data):
            response += f"  {group}:\n"
            response += f"    • Mean: {data.mean():.2f}\n"
            response += f"    • Std Dev: {data.std():.2f}\n"
            response += f"    • N: {len(data)}\n"

        response += f"\n**Test Results:**\n"
        response += f"  • F-statistic: {f_stat:.4f}\n"
        response += f"  • p-value: {p_value:.4f}\n\n"
        ...
        response += "  1. Data → Data Analysis → Anova: Single Factor\n"
        response += "  2. Select your data ranges\n"
        response += "  3. Click OK to get results\n\n"

        response += "💡 **Alternative:** Use =F.TEST() for two groups\n"

        return response


print("✅ Hypothesis Testing Engine created!")
...
class RegressionAnalysisEngine:
    ...
    def simple_linear_regression(self, df, x_col, y_col):
        ...
        response = "📈 **SIMPLE LINEAR REGRESSION**\n\n"
        response += f"**Model: {y_col} = f({x_col})**\n\n"

        response += "**Regression Equation:**\n"
        response += f"  {y_col} = {slope:.4f} × {x_col} + {intercept:.4f}\n\n"

        response += "**Model Statistics:**\n"
        response += f"  • Slope (m): {slope:.4f}\n"
        response += f"  • Intercept (b): {intercept:.4f}\n"
        response += f"  • R-squared (R²): {r2:.4f}\n"
        response += f"  • Correlation (r): {correlation:.4f}\n"
        response += f"  • RMSE: {rmse:.4f}\n\n"

        response += "**Interpretation:**\n"
        response += f"  • For every 1 unit increase in {x_col},\n"
        response += f"    {y_col} changes by {slope:.4f} units\n"
        response += f"  • R² = {r2:.4f} means the model explains {r2*100:.1f}% of variance\n"

        if r2 > 0.7:
            response += f"  • ✅ STRONG predictive power\n\n"
        elif r2 > 0.4:
            response += f"  • ⚠️ MODERATE predictive power\n\n"
        else:
            response += f"  • ❌ WEAK predictive power\n\n"

        # Excel formulas
        col_x = self.formula_gen.get_column_letter(x_col)
        col_y = self.formula_gen.get_column_letter(y_col)

        response += "📊 **Excel Formulas:**\n\n"
        response += f"**Slope:**\n"
        ...
        response += "💡 **Advanced: Use Analysis Toolpak**\n"
        response += "  Data → Data Analysis → Regression\n"
    ...
    def multiple_regression(self, df, x_cols, y_col):
        ...
        response = "📈 **MULTIPLE LINEAR REGRESSION**\n\n"
        ...
        response += f"  • Intercept: {intercept:.4f}\n"
        for i, col in enumerate(x_cols):
            response += f"  • {col}: {coefficients[i]:.4f}\n"

        response += f"\n**Model Performance:**\n"
        response += f"  • R-squared (R²): {r2:.4f}\n"
        response += f"  • Adjusted R²: {1 - (1-r2)*(len(y)-1)/(len(y)-len(x_cols)-1):.4f}\n"
        response += f"  • RMSE: {rmse:.4f}\n"
        response += f"  • Model explains {r2*100:.1f}% of variance\n\n"

        response += "**Interpretation:**\n"
        for i, col in enumerate(x_cols):
            response += f"  • {col}: For every 1 unit increase, {y_col} changes by {coefficients[i]:.4f}\n"

        response += "\n📊 **Excel Formula:**\n"
        response += "Use Analysis Toolpak:\n"
        response += "  1. Data → Data Analysis → Regression\n"
        response += "  2. Select Y range (dependent variable)\n"
        response += "  3. Select X range (all independent variables)\n"
        response += "  4. Click OK\n\n"

        response += "💡 **Or use LINEST function (advanced):**\n"
    ...
    def predict_values(self, model, x_col_name, x_value):
        ...
        response += "📈 **Excel Formula to predict:**\n"
        response += f"=FORECAST({x_value}, y_range, x_range)\n"

        return response


print("✅ Regression Analysis Engine created!")
...
class TimeSeriesAnalysisEngine:
    ...
    def time_series_summary(self, df, date_col, value_col):
        ...
        response += f"  • Start Date: {df[date_col].min()}\n"
        response += f"  • End Date: {df[date_col].max()}\n"
        response += f"  • Duration: {(df[date_col].max() - df[date_col].min()).days} days\n"
        response += f"  • Data Points: {len(df)}\n\n"
        ...
        if slope > 0:
            response += f"  • ↗️ UPWARD trend (slope: {slope:.4f})\n"
        elif slope < 0:
            response += f"  • ↘️ DOWNWARD trend (slope: {slope:.4f})\n"
        else:
            response += f"  • ➡️ NO clear trend\n"

        response += f"  • Average value: {df[value_col].mean():.2f}\n"
        response += f"  • Overall change: {df[value_col].iloc[-1] - df[value_col].iloc[0]:.2f}\n\n"
        ...
        response += "💡 **Visual Analysis in Excel:**\n"
        response += "  1. Select your date and value columns\n"
        response += "  2. Insert → Line Chart\n"
        response += "  3. Right-click line → Add Trendline\n"
    ...
    def calculate_moving_average(self, df, value_col, window=7):
        ...
        response = f"📊 **MOVING AVERAGE ({window}-period)**\n\n"
        ...
        response += "💡 **Use Cases:**\n"
        response += "  • Smooth out short-term fluctuations\n"
        response += "  • Identify trends more clearly\n"
        response += "  • Common windows: 7 (weekly), 30 (monthly)\n"

        return response


print("✅ Time Series Analysis Engine created!")
...
class PivotTableGenerator:
    ...
    def create_multi_level_pivot(self, df, index_cols, values_col, aggfunc='mean'):
        ...
        response = "📊 **MULTI-LEVEL PIVOT TABLE**\n\n"
        ...
        response += "💡 **Excel Pivot Table:**\n"
        response += "  1. Insert → PivotTable\n"
...
print("✅ Pivot Table Generator created!")
...
class ExcelFormulaBuilder:
    ...
    def build_vlookup(self, lookup_value, table_range, return_column, exact_match=True):
        ...
        response = "🔍 **VLOOKUP FORMULA BUILDER**\n\n"
        response += "**Your Formula:**\n"
        response += f"=VLOOKUP({lookup_value}, {table_range}, {return_column}, {match_type})\n\n"

        response += "**Breakdown:**\n"
        response += f"  • Lookup Value: {lookup_value} (what you're searching for)\n"
        response += f"  • Table Range: {table_range} (where to search)\n"
        response += f"  • Return Column: {return_column} (which column to return)\n"
        response += f"  • Exact Match: {match_type} (FALSE = exact, TRUE = approximate)\n\n"

        response += "📝 **Example Use Case:**\n"
        ...
        response += "💡 **Pro Tips:**\n"
        response += "  • Always use absolute references for table: $A$2:$E$100\n"
        response += "  • Lookup column MUST be the first column in range\n"
        response += "  • Use FALSE for exact matches (most common)\n"
        response += "  • Wrap in IFERROR to handle missing values:\n"
        response += f"    =IFERROR(VLOOKUP({lookup_value}, {table_range}, {return_column}, {match_type}), \"Not Found\")\n\n"

        response += "⚠️ **Common Errors:**\n"
        response += "  • #N/A: Value not found (use IFERROR)\n"
        response += "  • #REF!: Column number exceeds table range\n"
        response += "  • #VALUE!: Lookup value type mismatch\n"
    ...
    def build_index_match(self, lookup_value, lookup_range, return_range, match_type=0):
        ...
        response = "🎯 **INDEX-MATCH FORMULA BUILDER**\n\n"
        response += "**Your Formula:**\n"
        response += f"=INDEX({return_range}, MATCH({lookup_value}, {lookup_range}, {match_type}))\n\n"

        response += "**Breakdown:**\n"
        response += f"  • MATCH finds position of {lookup_value} in {lookup_range}\n"
        response += f"  • INDEX returns value from {return_range} at that position\n"
        response += f"  • Match Type: {match_type} ({match_types[match_type]})\n\n"
        ...
        response += "✅ **Why INDEX-MATCH is Better than VLOOKUP:**\n"
        response += "  • Can look LEFT (VLOOKUP can only look right)\n"
        response += "  • Faster for large datasets\n"
        response += "  • More flexible (separate lookup and return ranges)\n"
        response += "  • Doesn't break when columns are inserted/deleted\n\n"

        response += "💡 **Two-way Lookup (Row & Column):**\n"
    ...
    def build_xlookup(self, lookup_value, lookup_array, return_array, if_not_found="Not Found"):
        ...
        """
        response = "🚀 **XLOOKUP FORMULA BUILDER** (Excel 365+)\n\n"
        response += "**Your Formula:**\n"
        response += f'=XLOOKUP({lookup_value}, {lookup_array}, {return_array}, "{if_not_found}")\n\n'

        response += "**Breakdown:**\n"
        response += f"  • Lookup Value: {lookup_value}\n"
        response += f"  • Lookup Array: {lookup_array} (where to search)\n"
        response += f"  • Return Array: {return_array} (what to return)\n"
        response += f"  • If Not Found: \"{if_not_found}\" (default message)\n\n"

        response += "✨ **Why XLOOKUP is the Best:**\n"
        response += "  • Replaces VLOOKUP, HLOOKUP, INDEX-MATCH\n"
        response += "  • Built-in error handling\n"
        response += "  • Can search from bottom to top\n"
        response += "  • Can return multiple columns\n"
        response += "  • Simpler syntax than alternatives\n\n"
        ...
        response += "⚠️ **Note:** XLOOKUP only works in Excel 365 and Excel 2021+\n"
    ...
    def build_if_formula(self, condition, value_if_true, value_if_false):
        ...
        """
        response = "❓ **IF FORMULA BUILDER**\n\n"
        response += "**Your Formula:**\n"
        response += f'=IF({condition}, "{value_if_true}", "{value_if_false}")\n\n'

        response += "**Breakdown:**\n"
        response += f"  • Condition: {condition}\n"
        response += f"  • If TRUE: {value_if_true}\n"
        response += f"  • If FALSE: {value_if_false}\n\n"
        ...
        response += "💡 **Better Alternative: IFS (Excel 2016+)**\n"
        response += '  =IFS(A2>=90, "A", A2>=80, "B", A2>=70, "C", TRUE, "F")\n'
        response += "  Cleaner than nested IFs!\n\n"

        response += "🎯 **Common Use Cases:**\n"
        response += "  • Grade assignment: IF(score>=passing, ...)\n"
        response += "  • Category labeling: IF(value>threshold, ...)\n"
        response += "  • Status checks: IF(date>TODAY(), ...)\n"
        response += "  • Commission calculation: IF(sales>target, ...)\n"
    ...
    def build_sumifs(self, sum_range, criteria_ranges, criteria_values):
        ...
        """
        response = "➕ **SUMIFS FORMULA BUILDER**\n\n"
        ...
        response += "**Breakdown:**\n"
        response += f"  • Sum Range: {sum_range} (what to sum)\n"
        for i, (crit_range, crit_val) in enumerate(zip(criteria_ranges, criteria_values), 1):
            response += f"  • Criteria {i}: {crit_range} = {crit_val}\n"
        ...
        response += "🎯 **Common Conditions:**\n"
        response += '  • Exact match: "Sales"\n'
        response += '  • Greater than: ">100"\n'
        response += '  • Less than: "<50"\n'
        response += '  • Not equal: "<>Value"\n'
        response += '  • Contains: "*text*"\n'
        response += '  • Date range: ">="&DATE(2024,1,1)\n\n'
    ...
    def build_conditional_formatting_guide(self, rule_type):
        ...
        """
        response = "🎨 **CONDITIONAL FORMATTING GUIDE**\n\n"

        if rule_type.lower() == "highlight":
            response += "**HIGHLIGHT CELLS RULES**\n\n"
            response += "1️⃣ **Greater Than:**\n"
            response += "   • Home → Conditional Formatting → Highlight Cells Rules → Greater Than\n"
            response += "   • Formula: =A1>100\n"
            response += "   • Use: Highlight sales above target\n\n"

            response += "2️⃣ **Less Than:**\n"
            response += "   • Formula: =A1<50\n"
            response += "   • Use: Flag low inventory\n\n"

            response += "3️⃣ **Between:**\n"
            response += "   • Formula: =AND(A1>=50, A1<=100)\n"
            response += "   • Use: Highlight mid-range values\n\n"

            response += "4️⃣ **Equal To:**\n"
            response += "   • Formula: =A1=\"Complete\"\n"
            response += "   • Use: Highlight specific status\n\n"

            response += "5️⃣ **Text Contains:**\n"
            response += "   • Formula: =ISNUMBER(SEARCH(\"important\", A1))\n"
            response += "   • Use: Find keywords\n\n"

            response += "6️⃣ **Duplicate Values:**\n"
            response += "   • Formula: =COUNTIF($A$1:$A$100, A1)>1\n"
            response += "   • Use: Find duplicates\n\n"

        elif rule_type.lower() == "databar":
            response += "**DATA BARS**\n\n"
            response += "📊 Visual bars showing relative values\n\n"
            response += "**Setup:**\n"
            response += "  1. Select range\n"
            response += "  2. Conditional Formatting → Data Bars\n"
            response += "  3. Choose color scheme\n\n"
            response += "💡 **Best For:**\n"
            response += "  • Sales performance comparison\n"
            response += "  • Progress tracking\n"
            response += "  • Budget utilization\n\n"

        elif rule_type.lower() == "colorscale":
            response += "**COLOR SCALES**\n\n"
            response += "🌈 Gradient colors based on values\n\n"
            response += "**Setup:**\n"
            response += "  1. Select range\n"
            response += "  2. Conditional Formatting → Color Scales\n"
            response += "  3. Choose: 2-color or 3-color scale\n\n"
            response += "💡 **Best For:**\n"
            response += "  • Heat maps\n"
            response += "  • Performance matrices\n"
            response += "  • Risk assessment tables\n\n"

        elif rule_type.lower() == "iconset":
            response += "**ICON SETS**\n\n"
            response += "🚦 Traffic lights, arrows, ratings\n\n"
            response += "**Setup:**\n"
            response += "  1. Select range\n"
            response += "  2. Conditional Formatting → Icon Sets\n"
            response += "  3. Choose icon style\n\n"
            response += "💡 **Best For:**\n"
            response += "  • Status indicators (🟢🟡🔴)\n"
            response += "  • Trend arrows (⬆️➡️⬇️)\n"
            response += "  • Star ratings (⭐⭐⭐)\n\n"

        elif rule_type.lower() == "custom":
            response += "**CUSTOM FORMULAS**\n\n"
            response += "🔧 Advanced conditional formatting\n\n"
        ...
        response += "  • Use $ for absolute references in formulas\n"
        response += "  • Test on small range first\n"
        response += "  • Manage rules: Home → Conditional Formatting → Manage Rules\n"
        response += "  • Clear rules: Clear Rules → Clear Rules from Selected Cells\n"
    ...
    def build_array_formula(self, formula_type):
        ...
        if formula_type.lower() == "sum":
            ...
            response += "  Use: Calculate total revenue (quantity × price)\n\n"
        ...
        response += "\n⚡ **Array Formula Tips:**\n"
        response += "  • Excel 365: Formulas spill automatically\n"
        response += "  • Older Excel: Press Ctrl+Shift+Enter\n"
        response += "  • Use {} brackets for manual arrays: {1,2,3}\n"
        response += "  • Combine with other functions for power!\n"

        return response


print("✅ Excel Formula Builder created!")
...
class MasterExcelChatBot(UltraAdvancedChatBot):
        ...
        try:
            ...
            if 't-test' in user_lower or 't test' in user_lower or 'ttest' in user_lower:
                if 'paired' in user_lower and len(columns) >= 2:
                    ...
                    response = "❌ For t-test, specify: 't-test [value_col] between [group1] and [group2] in [group_col]'\n"
                    response += "Example: 't-test Salary between Sales and IT in Department'"
                else:
                    response = "❌ Please specify columns for t-test"
            ...
            # === TIME SERIES ===
                ...
                if date_cols and columns:
                    ...
                    response = f"📅 Date columns found: {', '.join(date_cols)}\nSpecify a value column for analysis"
                else:
                    response = "❌ No date columns detected in dataset"
                ...
                if columns:
                    ...
                    response = "❌ Specify column for moving average\nExample: 'moving average of Sales'"
            ...
            # === PIVOT TABLES ===
                ...
                if 'multi' in user_lower or 'multiple' in user_lower:
                    if len(columns) >= 3:
                        ...
                        response = "❌ Multi-level pivot needs at least 2 index columns and 1 value column"
                elif len(columns) >= 2:
                    # Two-dimensional pivot
                    if len(columns) == 2:
            ...
            # === FORMULA BUILDER ===
                ...
                response += "\n\n💡 Replace with your actual column references!"
                ...
                response += "\n\n💡 Customize condition and values for your needs!"
                ...
                response += "\n\n💡 Adjust ranges and criteria for your data!"
    ...
    def get_master_help_message(self):
        ...
        help_text = """
🤖 **MASTER EXCEL ANALYSIS ASSISTANT**
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
📊 **COMPREHENSIVE STATISTICS:**
  ...
  • "Confidence interval for [column]"

🔬 **HYPOTHESIS TESTING:**
  • "t-test [value_col] between [group1] and [group2] in [group_col]"
    Example: "t-test Salary between Sales and IT in Department"
  • "Paired t-test [col1] and [col2]"
  • "Chi-square test between [col1] and [col2]"
  • "ANOVA test [value_col] by [group_col]"

📈 **REGRESSION ANALYSIS:**
  • "Regression [x_col] vs [y_col]" - Simple linear regression
  • "Multiple regression [x1] [x2] vs [y]" - Multiple regression
  • Example: "Regression Age vs Salary"
  • Example: "Multiple regression Age Experience vs Salary"

📅 **TIME SERIES ANALYSIS:**
  • "Time series analysis of [column]" - Trend analysis
  • "Moving average of [column]" - Smoothing
  • "7-day moving average of [column]" - Specify window

📊 **PIVOT TABLES:**
  • "Pivot table [rows]" - Simple count
  • "Pivot table [rows] by [columns]" - Cross-tabulation
  • "Pivot table [rows] by [columns] with [values]" - Aggregation
  • "Multi-level pivot [row1] [row2] with [value]"

🔧 **FORMULA BUILDER:**
  • "Build VLOOKUP" - VLOOKUP formula guide
  • "Build INDEX MATCH" - INDEX-MATCH formula
  • "Build XLOOKUP" - XLOOKUP guide (Excel 365)
  • "Build IF formula" - IF statement builder
  • "Build SUMIFS" - Conditional sum formulas
  • "Conditional formatting guide" - Formatting rules
  • "Array formula guide" - Advanced arrays

🔍 **DATA ANALYSIS:**
  • "Check for missing values"
  • "Calculate correlations"
  • "Find outliers in [column]"
  • "Compare [numeric] across [group]"
  • "Group by [column]"
...
🔒 **Privacy:** 100% offline analysis. Your data never leaves your computer!
...
print("✅ Master Excel ChatBot with ALL features created!")

# INITIALIZE THE MASTER CHATBOT

bot = MasterExcelChatBot()
print("✅ Master Excel ChatBot initialized!")
print("✅ All advanced features loaded!")
print()
print("📦 **Available Features:**")
print("  ✓ Comprehensive Statistics")
print("  ✓ Hypothesis Testing (t-test, chi-square, ANOVA)")
print("  ✓ Regression Analysis (Simple & Multiple)")
print("  ✓ Time Series Analysis")
print("  ✓ Pivot Tables")
print("  ✓ Excel Formula Builder")
print("  ✓ Filtering & Grouping")
...
print("="*70)
print("🚀 MASTER EXCEL ANALYSIS ASSISTANT")
print("="*70)
print()
...
print("🧪 TESTING HYPOTHESIS TESTING FEATURES\n")
...
def show_quick_reference():
    ...
    reference = """
╔═══════════════════════════════════════════════════════════════════╗
║           MASTER EXCEL ASSISTANT - QUICK REFERENCE               ║
╚═══════════════════════════════════════════════════════════════════╝
...
╔═══════════════════════════════════════════════════════════════════╗
║  📖 TIP: Every response includes Excel formulas you can copy!     ║
...
✅ BASIC STATISTICS (15+ metrics):
   • Mean, Median, Mode
   • Standard Deviation, Variance, Range
   • Min, Max, Quartiles, IQR
   • Skewness, Kurtosis
   • Percentiles (10th to 99th)
   • Z-Scores
...
✅ PIVOT TABLES (3 types):
   • Simple pivot (1D)
   • Cross-tabulation (2D)
   • Multi-level pivot (3D+)

✅ FORMULA BUILDER (20+ formulas):
   • VLOOKUP with error handling
   • INDEX-MATCH (better than VLOOKUP)
   • XLOOKUP (Excel 365)
   • IF, IFS, nested IF
   • SUMIFS, COUNTIFS, AVERAGEIFS
   • Array formulas (UNIQUE, FILTER, SORT)
...
✅ EXCEL INTEGRATION:
   • Every analysis includes Excel formulas
   • Copy-paste ready formulas
   • Cell reference examples
   • Step-by-step Excel instructions

╔═══════════════════════════════════════════════════════════════════╗
║  📊 TOTAL: 100+ Statistical Functions & Analysis Tools           ║
║  🔧 TOTAL: 50+ Excel Formula Templates                           ║
║  📈 TOTAL: 10+ Visualization Options                             ║
║  🧠 NATURAL LANGUAGE UNDERSTANDING for easy interaction          ║
╚═══════════════════════════════════════════════════════════════════╝

🎯 NEXT STEPS TO COMPLETE THE APP:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. ✅ Core Backend - COMPLETED! (100%)
2. ⏳ Desktop GUI - ChatGPT-like interface (Next)
3. ⏳ Export Features - PDF reports, Excel exports
4. ⏳ Package as .exe - Desktop application

Current Status: Backend is 100% complete and fully functional!
Ready to build the desktop UI! 🚀
"""
    print(summary)
...
class SimpleExportEngine:
    ...
    def export_to_text(self, content, filename="analysis_report.txt", title="Excel Analysis Report"):
        ...
        try:
            ...
            response = f"✅ **Text Report Export Successful!**\n\n"
            response += f"📄 File: {filename}\n"
            response += f"🕐 Timestamp: {timestamp}\n"
            response += f"💾 Format: Plain Text (UTF-8)\n\n"
            response += f"💡 **Location:** ./{filename}\n"
            response += f"📝 **Can be opened with:** Any text editor, Word, Notepad\n\n"
            response += f"📈 **Excel Formula:** (Text export is programmatic)\n"
...
"""
            ...
            response = f"✅ **HTML Report Export Successful!**\n\n"
            response += f"📄 File: {filename}\n"
            response += f"🕐 Timestamp: {timestamp}\n"
            response += f"💾 Format: HTML (Web Page)\n\n"
            response += f"💡 **Location:** ./{filename}\n\n"
            ...
            response += f"✨ **Beautiful styled report with colors and formatting!**\n"
            ...
            return f"❌ HTML Export Error: {str(e)}"
    ...
    def export_to_excel(self, df, filename="exported_data.xlsx", sheet_name="Data"):
        ...
        try:
            ...
            response = f"✅ **Excel Export Successful!**\n\n"
            response += f"📊 File: {filename}\n"
            response += f"📋 Sheet: {sheet_name}\n"
            response += f"📈 Rows: {len(df)}\n"
            response += f"📈 Columns: {len(df.columns)}\n"
            response += f"🕐 Timestamp: {timestamp}\n\n"
            response += f"✨ **Formatting Applied:**\n"
            response += f"  • Blue header with white text\n"
            response += f"  • Auto-adjusted column widths\n"
            response += f"  • Professional borders\n"
            response += f"  • Center-aligned headers\n\n"
            response += f"💡 **Location:** ./{filename}\n\n"
            response += f"📈 **Open in Excel and enjoy your formatted data!**\n"
    ...
    def export_to_csv(self, df, filename="exported_data.csv"):
        ...
        try:
            ...
            response = f"✅ **CSV Export Successful!**\n\n"
            response += f"📄 File: {filename}\n"
            response += f"📈 Rows: {len(df)}\n"
            response += f"📈 Columns: {len(df.columns)}\n"
            response += f"🕐 Timestamp: {timestamp}\n\n"
            response += f"💡 **Location:** ./{filename}\n\n"
            response += f"📊 **CSV can be opened in:**\n"
            response += f"  • Excel\n"
            response += f"  • Google Sheets\n"
            response += f"  • Any text editor\n"
            response += f"  • Python/Pandas\n"
    ...
    def export_to_markdown(self, content, filename="analysis_report.md", title="Excel Analysis Report"):
        ...
        try:
            ...
            response = f"✅ **Markdown Export Successful!**\n\n"
            response += f"📄 File: {filename}\n"
            response += f"🕐 Timestamp: {timestamp}\n"
            response += f"💾 Format: Markdown\n\n"
            response += f"💡 **Location:** ./{filename}\n\n"
            response += f"📝 **Can be viewed in:**\n"
            response += f"  • VS Code\n"
            response += f"  • GitHub\n"
            response += f"  • Markdown viewers\n"
            response += f"  • Converted to PDF/HTML\n"
    ...
    def show_export_history(self):
        ...
        if not self.export_history:
            return "📋 No exports yet in this session."

        response = "📋 **Export History**\n\n"
...
print("✅ Simple Export Engine created (works without reportlab)!")
        ...
        if 'export' in user_lower or 'save' in user_lower:
            ...
            if 'html' in user_lower:
                ...
                if self.analyzer.df_filtered is not None:
                    ...
                    return "❌ No data to export. Load data first!"
...
print("✅ Full-Featured Excel ChatBot with exports created!")

# INITIALIZE WITH EXPORT CAPABILITIES

bot = FullFeaturedExcelChatBot()
print("✅ Excel ChatBot with Export Features initialized!")
print()
print("📦 **Export Formats Available:**")
print("  ✓ HTML (Save as PDF from browser)")
print("  ✓ Text (.txt)")
print("  ✓ Markdown (.md)")
print("  ✓ Excel (.xlsx) - with formatting")
print("  ✓ CSV (.csv)")
print()
...
deployment_guide = """
╔═══════════════════════════════════════════════════════════════════════╗
║          EXCEL ANALYSIS ASSISTANT - DEPLOYMENT GUIDE                  ║
╚═══════════════════════════════════════════════════════════════════════╝
...
After building, find:
  • Windows: dist\\ExcelAssistant.exe
  • Linux/Mac: dist/ExcelAssistant
...
✅ HTML Reports (can be saved as PDF from any browser)
✅ Text Reports (.txt)
✅ Markdown (.md)
...
**Usage:**
  ...
  • "export to text" - Plain text report
  • "export to markdown" - Markdown format
...
╔═══════════════════════════════════════════════════════════════════════╗
║  🎉 YOUR APP IS NOW COMPLETE AND READY TO DISTRIBUTE!                ║
╚═══════════════════════════════════════════════════════════════════════╝
"""
...
🔧 **EXCEL FORMULA BUILDER:**
   ✓ VLOOKUP with examples
   ✓ INDEX-MATCH (better alternative)
   ✓ XLOOKUP (Excel 365)
   ✓ IF, IFS, nested IF
   ✓ SUMIFS, COUNTIFS, AVERAGEIFS
   ✓ Array formulas (UNIQUE, FILTER, SORT)
   ✓ Conditional formatting guides
   ✓ 50+ formula templates

📈 **DATA OPERATIONS:**
   ✓ Filtering (7 operators)
   ✓ Grouping & aggregation
   ✓ Pivot tables (1D, 2D, multi-level)
   ✓ Sorting & ranking
   ✓ Missing value analysis
...
║  📦 Package it, distribute it, and help people analyze data!          ║
╚═══════════════════════════════════════════════════════════════════════╝
"""
...
<body>
    ...
    <div class="footer"><p>Excel Analysis Assistant</p><p>To save as PDF: Press Ctrl+P and select "Save as PDF"</p></div>
            ...
            return f"✅ HTML Report saved: {filename}\\n💡 Open in browser and press Ctrl+P to save as PDF"
        except Exception as e:
    ...
    def export_to_excel(self, df, filename="exported_data.xlsx", sheet_name="Data"):
        ...
        try:
            ...
            return f"✅ Excel file saved: {filename} ({len(df)} rows, {len(df.columns)} columns)"
        except Exception as e:
            return f"❌ Export Error: {str(e)}"
    ...
    def export_to_csv(self, df, filename="exported_data.csv"):
        ...
        try:
            ...
            return f"✅ CSV file saved: {filename}"
        except Exception as e:
            return f"❌ Export Error: {str(e)}"
...
class ExcelAssistantCLI:
    ...
    def run(self):
        ...
        print()
        print("💡 Features:")
        print("   • 100+ Statistical Functions")
        print("   • Natural Language Understanding")
        print("   • Excel Formula Builder")
        print("   • Export to HTML/Excel/CSV")
        print("   • 100% Offline & Secure")
        print()
        ...
        while self.running:
            try:
                ...
                if user_input.lower() in ['quit', 'exit', 'bye']:
                    print("\\n👋 Thank you for using Excel Analysis Assistant!")
                    self.running = False
                    break
...
print("✅ Complete standalone application saved: excel_assistant_complete.py")
...
class ExcelAssistantGUI:
    ...
    def setup_ui(self):
        ...
        self.add_bot_message(
            "👋 Welcome to Excel Analysis Assistant!\\n\\n"
            "I can help you analyze Excel files with natural language.\\n\\n"
            "📂 Click 'Load Excel File' to get started!\\n"
...
print("✅ GUI application saved: excel_assistant_gui.py")
...
cls
echo ╔═══════════════════════════════════════════════════════════════╗
echo ║     EXCEL ANALYSIS ASSISTANT - BUILD SCRIPT (Windows)        ║
echo ╚═══════════════════════════════════════════════════════════════╝
echo.
...
echo.
echo 💡 TIP: Test the executables before distribution
echo.
pause
'''
...
clear
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║     EXCEL ANALYSIS ASSISTANT - BUILD SCRIPT (Linux/Mac)      ║"
echo "╚═══════════════════════════════════════════════════════════════╝"
echo ""
...
print("   python excel_assistant_enhanced_gui.py")
print()
print("💡 Features to try:")
print("   1. Start typing 'show' - see auto-complete suggestions")
print("   2. Use arrow keys to navigate suggestions")
print("   3. Press Enter to select a suggestion")
print("   4. Click 'Show Help' to see built-in command guide")
print("   5. Search in help panel for specific commands")
print()
